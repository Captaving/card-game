    ...
    let html = `
        <div style="max-width: 1100px; margin: 0 auto;">
            <div style="text-align:center; margin-bottom: 15px;">
                <h2 style="margin:0; color:#d4af37;">Раунд ${state.round || 1} • Ход ${state.turn}</h2>
                <div style="display:flex; justify-content:center; gap:40px; margin:8px 0; font-size:1.1em;">
                    <div>Твои Мечи: <strong style="color:#4caf50;">${state.playerSwords || 0}</strong></div>
                    <div>Мечи Бота: <strong style="color:#ef5350;">${state.botSwords || 0}</strong></div>
                </div>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <div style="color:#ff8a80; font-weight:bold;">ПРОТИВНИК (Бот)</div>
                    <div style="color:#aaa; font-size:0.9em;">Рука: ${state.botHand.length} карт | Сброс: ${state.botDiscard ? state.botDiscard.length : 0}</div>
                </div>
                
                <div class="lanes" style="gap: 15px;">
                    <div class="lane left" style="border-color: #5c6bc0; background: rgba(30,25,40,0.6);">
                        <h4 style="color:#90caf9;">ЛЕВЫЙ ФЛАНГ</h4>
                        <div class="cards" id="bot-left"></div>
                    </div>
                    <div class="lane right" style="border-color: #ef5350; background: rgba(40,25,25,0.6);">
                        <h4 style="color:#ef9a9a;">ПРАВЫЙ ФЛАНГ</h4>
                        <div class="cards" id="bot-right"></div>
                    </div>
                </div>
            </div>

            <div style="height: 2px; background: linear-gradient(to right, transparent, #8b5a2b, transparent); margin: 15px 0;"></div>

            <!-- PLAYER AREA (bottom) -->
            <div>
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <div style="color:#64b5f6; font-weight:bold;">ТЫ</div>
                    <div style="color:#aaa; font-size:0.9em;">Рука: ${state.playerHand.length} карт | Сброс: ${state.playerDiscard ? state.playerDiscard.length : 0}</div>
                </div>
                
                <div class="lanes" style="gap: 15px;">
                    <div class="lane left" style="border-color: #42a5f5; background: rgba(25,35,55,0.6);">
                        <h4 style="color:#64b5f6;">ЛЕВЫЙ ФЛАНГ</h4>
                        <div class="cards" id="player-left"></div>
                    </div>
                    <div class="lane right" style="border-color: #ef5350; background: rgba(55,30,30,0.6);">
                        <h4 style="color:#ef9a9a;">ПРАВЫЙ ФЛАНГ</h4>
            ...
            <!-- HAND -->
            <div class="hand" style="margin-top: 20px; background: rgba(40,30,20,0.7); border: 2px solid #8b5a2b;">
                <h4 style="color:#d4af37; margin-bottom: 10px;">Твоя рука (${state.playerHand.length} карт)</h4>
                <div class="cards" id="player-hand" style="min-height: 170px;"></div>
            </div>

            <!-- BUTTONS -->
            <div style="text-align:center; margin-top: 20px; display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                <button onclick="endPlayerTurn()" style="background:#2e7d32; border-color:#66bb6a; padding:14px 35px;">Завершить ход</button>
                <button onclick="playerPass()" style="background:#5d4037; border-color:#a1887f; padding:14px 35px;">ПАСС (завершить раунд)</button>
                <button onclick="surrenderGame()" style="background:#c62828; border-color:#ef5350; padding:12px 25px; font-size:0.95em;">Сдаться</button>
            </div>
        </div>
    `;
...
function renderPlayerHand() {
    ...
    state.playerHand.forEach((card, idx) => {
        ...
        el.style.borderColor = '#42a5f5'; // Blue for player
        el.onclick = () => showLaneSelection(idx);
        container.appendChild(el);
    });
}

function showLaneSelection(handIndex) {
    const state = botGameState;
    ...
    const selectionHTML = `
        <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.85); display: flex; align-items: center; justify-content: center; z-index: 300;">
            <div style="background: #2c2118; padding: 30px; border-radius: 16px; border: 3px solid #8b5a2b; text-align: center; max-width: 420px;">
                <h3 style="color:#d4af37; margin-bottom: 20px;">Куда поставить "${card.name}"?</h3>
                
                <div style="display: flex; gap: 20px; justify-content: center;">
                    ${allowed.includes('left') ? 
                        `<button onclick="playCardToLane(${handIndex}, 'left'); this.closest('.modal-like').remove();" 
                            style="padding: 18px 40px; font-size: 1.1em; background: #1565c0; border-color: #42a5f5;">ЛЕВЫЙ ФЛАНГ</button>` : ''}
                    
                    ${allowed.includes('right') ? 
                        `<button onclick="playCardToLane(${handIndex}, 'right'); this.closest('.modal-like').remove();" 
                            style="padding: 18px 40px; font-size: 1.1em; background: #c62828; border-color: #ef5350;">ПРАВЫЙ ФЛАНГ</button>` : ''}
                </div>
                
                <button onclick="this.closest('div[style*=\'position: fixed\']').remove()" 
                    style="margin-top: 25px; background: #5d4037; border-color: #a1887f; padding: 10px 30px;">Отмена</button>
            </div>
        </div>
    `;
    
    const wrapper = document.createElement('div');
    wrapper.className = 'modal-like';
    wrapper.innerHTML = selectionHTML;
    document.body.appendChild(wrapper);
}

function playCardToLane(handIndex, lane) {
    // Close any open modals
    document.querySelectorAll('.modal-like').forEach(el => el.remove());
    
    const state = botGameState;
    
    if (state.hasPlayedThisTurn) {
        showToast("За этот ход можно выложить только 1 карту!");
        return;
    }
    
    const card = state.playerHand[handIndex];
    
    const targetArray = lane === 'left' ? state.playerLeft : state.playerRight;
    
    targetArray.push(card);
    state.playerHand.splice(handIndex, 1);
    state.hasPlayedThisTurn = true;   // Block second card this turn
    
    // Apply on-play effect immediately
    applyCardEffect(card, 'player', lane);
    
    showToast(`Вы сыграли ${card.name} на ${lane === 'left' ? 'Левый' : 'Правый'} фланг`);
    renderGameBoard();
}

function playerPass() {
    const state = botGameState;
    showToast("Ты пасанул. Бот продолжает...");
    
    // Bot plays one more time then we can end round or let bot pass too
    // For simplicity in this version:
    endPlayerTurn(); // reuse logic
}
...
function endPlayerTurn() {
    ...
    state.hasPlayedThisTurn = false; // Reset for next turn
    
    // Bot plays 1 card (since we limit to 1 per turn)
    botPlayTurn();
    
    // Resolve lanes (show power)
    resolveLanes();
    
    state.turn++;
    
    // Simple end condition
    if (state.turn > 8 || (state.playerHand.length === 0 && state.playerDeck.length === 0)) {
        endBotGame();
        return;
    }
    ...
    renderGameBoard();
    showToast(`Ход ${state.turn}. Теперь твой ход.`);
}
