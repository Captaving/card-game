...
const SERVER_URL = 'wss://YOUR_SERVER_ADDRESS/ws'; // ← замените
...
const Game = {
  ...
  // ─── UI bindings ──────────────────────────────────────────────────────────
  _bindUI() {
    el('btn-find-match')?.addEventListener('click', () => this.findMatch());
    el('btn-cancel-queue')?.addEventListener('click', () => this.cancelQueue());
    el('btn-pass')?.addEventListener('click', () => this.pass());
    el('btn-forfeit')?.addEventListener('click', () => this.forfeit());
    el('btn-play-again')?.addEventListener('click', () => showScreen('screen-main'));
    el('btn-deck-builder')?.addEventListener('click', () => DB.open());

    // Bot game
    el('btn-vs-bot')?.addEventListener('click', () => {
      const saved = DB.getSaved();
      if (!saved) { Anim.toast('Сначала соберите колоду'); return; }
      this._showBotPicker(saved);
    });
    ...
    ['frow-me-left','frow-me-right'].forEach(rowId => {
      el(rowId)?.addEventListener('click', e => {
        if (e.target !== el(rowId) && e.target !== el(rowId).querySelector('.row-label')) return;
        if (!GameUI.selectedCard) return;
        const row = rowId.includes('left') ? 'left' : 'right';
        GameUI._triggerPlay({ ...GameUI.selectedCard, row });
      });
    });
  ...
  _renderLocal() {
    ...
    el('hand-container').querySelectorAll('.card').forEach(cardEl => {
      cardEl.onclick = async e => {
        ...
        if (!card || s.turn !== 'player' || me.passed) return;

        let row = card.row;
        if (row === 'any' || card.type === 'special') {
          row = await GameUI._pickRow();
          if (!row) return;
        }
        const ti = await GameUI._gatherTargets({ ...card, currentPower:card.currentPower||card.power }, row);
        if (ti === null) return;
